...
class Game:
    def __init__(self):
        ...
        self.font_items = pygame.font.Font("assets/fonts/Pixelation.ttf", 15)
        ...
        while True:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if self.map_id == "background":
                        box_collected = self.boxes.get_box(event, flashs_list, self.player, self.inventory, glowsticks_list)
                        if box_collected:
                            texts = utils.update_qty_items(self.font, self.inventory.get_inventory())
                        self.player.control(event, self.maze_map)

                        item_used = self.inventory.use_item(event)
                        if item_used:
                            texts = utils.update_qty_items(self.font, self.inventory.get_inventory())
                        ...
                        #print(self.billy.position())
                        #print(monsters_captured[0])
                        #print(self.player.position_maze)

                        #print(f"got_killed: {got_killed}")
                        #print(flashs_list)
                    ...
                    if len(glowsticks_list) > 0:
                        for glowstick_position in glowsticks_list:
                            for column, line in glowstick_position:
                                glow_cells.append((line + 1, column + 1))
                                
                    cells.extend(flash_cells)
                    cells.extend(glow_cells)
            ...
            if self.map_id in ["home_screen", "config"]:
                ...
                if not got_killed:
                    ...
                    if self.billy.pos in glow_cells:
                        self.billy.draw()
                    if self.bob.pos in glow_cells:
                        self.bob.draw()
                    
                    self.maze.display_maze_cells(self.screen, cells)
                else:
                    self.screen.fill("Red")
                    text = self.font.render("SE FODEU", None, "White")
